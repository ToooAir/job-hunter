...
### Step A 結果（2026-09-05）：**三項判準全數不通過 → Step B 不做，翻譯留著**

腳本 `scripts/experiment_translation_retirement.py`（可重跑）。樣本 40 筆德文 JD，
A/B 與 C 各半（母體 86% 是 C，不分層的話「等級一致」會因為兩邊都說 C 而變成廢話），
排除 regex 閘已攔下的（那些本來就進不到 LLM）。成本 $0.1022，與估計相符。

| 判準 | 門檻 | 實測 | 結果 |
|---|---|---|---|
| KB top-5 重疊 | 中位 ≥ 4/5 | **中位 3/5**、平均 2.80、分布只有 {2: 8, 3: 32}，從沒到過 4 | ✗ |
| 等級一致 | ≥ 36/40 | **34/40**；**A/B 層只有 14/20**，C 層 20/20 | ✗ |
| 語言標籤 | 原文版錯誤 ≤ 譯文版 | 原文版 2 個硬錯，譯文版 0 | ✗ |

- **檢索**：德文查詢 top-1 中位 0.410（最低 0.329）對英文 0.450（最低 0.337）；0.35 門檻下
  **3/40 德文查詢會拿到空 context**，英文只有 1 筆。跨語言檢索衰減是真的，不是雜訊。
- **評分有方向性偏誤**：6 次等級翻轉有 5 次是「德文原文評得比較低」（B vs A ×4、C vs B ×1），
  只有 1 次相反。退役翻譯＝系統性少發 A＝供給變少。
  （注意：兩臂的 JD 語言與檢索到的 context 同時不同，這個實驗**沒有把兩個效應分開**；
  但對決策無影響——不論成因，退役都會少 A。）
- **推翻了本項的立論**：原假設是「翻譯把 Deutsch C1 翻掉，是 hallucinated de_required 的來源」。
  實測相反，兩個硬錯都出在**德文原文那一臂**：
  - `alphacoders` Senior Data & AI Engineer：JD 全文 **0 個語言字眼**，德文臂憑空給 `de_required`，
    直接把它壓成 C（英文臂 `en_required`/B 68）。
  - `BettercallPaul`：JD 寫著「sicheres Deutsch in Wort und Schrift」，德文臂只給 `de_plus`，**漏抓**；
    英文臂正確給 `de_required`。
- 另 5 筆是「德文臂 de_plus vs 英文臂 unknown」，屬於資訊量差異而非錯誤（全德文 JD 判 de_plus 合理），
  兩者都不改變決策。

**結論**：翻譯步驟保留。每天 ~100 次 nano 呼叫（$0.02/天量級）買到的是更好的檢索、更高的 A 產出
與更準的語言標籤，這筆帳划算。要重新評估時換模型重跑同一支腳本即可，不必重新爭論。
